...
const VocabularyPage = () => (
    <div className="min-h-screen bg-gradient-to-br from-indigo-100 via-white to-cyan-100">
      <div className="container mx-auto px-4 py-8">
        <h2 className="text-3xl font-bold text-gray-800 mb-6 flex items-center">
          <BookOpen className="w-8 h-8 mr-2 text-blue-500" />
          Vocabulary
        </h2>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {courseData[currentSubTab].vocabulary.map((item, index) => (
            <FlashCard 
              key={index} 
              term={item.term} 
  ...
  const ExercisesPage = () => (
    <div className="min-h-screen bg-gradient-to-br from-indigo-100 via-white to-cyan-100">
      <div className="container mx-auto px-4 py-8">
        <h2 className="text-3xl font-bold text-gray-800 mb-6 flex items-center">
          <Play className="w-8 h-8 mr-2 text-green-500" />
          Exercises
        </h2>
        <div className="space-y-6">
          {courseData[currentSubTab].exercises.map((exercise, index) => (
            <QuizQuestion 
              key={index} 
              ...
              onAnswer={(answer) => {
                ...
                setCelebrationType(isCorrect ? 'correct' : 'wrong');
                playSound(isCorrect ? 'correct' : 'wrong');
                setShowCelebration(true);
              }} 
              userAnswer={null} // Placeholder for user answer
              showResult={false} // Placeholder for showing result
            />
          ))}
        </div>
      </div>
